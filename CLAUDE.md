...
- MoonBoard 2024 setup: full-size board, 11 columns (A-K) x 18 rows (1-18), 198 holds.
...
1. [x] **Service + characteristic UUIDs.** Standard Nordic UART Service. Service `6E400001-...`; write target is the RX characteristic `6E400002-...`. Board advertises as "MoonBoard A".
2. [x] **0- vs 1-based.** **0-based**, range 0-197. (`positionsAreOneBased` alternative removed.)
3. [x] **Mapping direction.** **Serpentine column-major** (A=0..K=10, rows r=1..18): `position = c*18 + (r-1)` for even column index `c`, `c*18 + (18-r)` for odd `c`. Even columns run bottom-up, odd columns run top-down. Verified 0->A1, 17->A18, 50->C15, **58->D14, 67->D5**, 197->K18. (Initial recon read it as straight column-major because A1/C15/K18 all fall in even columns, where both layouts agree.)
4. [x] **Framing / MTU / write mode.** No extra framing beyond `l#...#`, but the box is an old Nordic chip with a **23-byte ATT MTU** - writes over 20 bytes are silently dropped. The app chunks the command into <=20-byte pieces and writes them **sequentially with write-with-response**, waiting for each ack before the next; the framing lets the box reassemble.
...
1. **Board** (main): 11x18 tappable grid. Tap a cell to cycle: none -> start (green) -> move (blue) -> end (red) -> none. Buttons: "Light it" (send to board), "Clear", "Save preset". Show connection status (disconnected / scanning / connected + peripheral name).
...
- Device deploys are done by the human via Xcode (CmdR). Don't attempt device installs.
